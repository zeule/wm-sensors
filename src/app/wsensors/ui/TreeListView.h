...
//
// Partly implemented from a MFC CTreeListView control by Gerolf K�hnel
...
// Adaptation for modern compilers by Eugene Shalygin (2022)
//
...
typedef struct tagTLVITEM {
	...
	UINT stateMask;
	UINT format;
	LPTSTR pszText; // TODO replace with std::string to avoid re-allocations
	UINT cchTextMax;
	int iImage;
	COLORREF clrText;
	COLORREF clrBack;
...
class ATL_NO_VTABLE CTreeListViewImpl: public CWindowImpl<T, TBase, TWinTraits>, public CCustomDraw<T> {
public:
	...
	BOOL SetSubItem(HTREEITEM hItem, const LPTLVITEM pItem)
	{
		...
		if (pItem->mask & TLVIF_TEXT) {
			...
			ATLTRY(pItemT->pszText = new TCHAR[static_cast<std::size_t>(::lstrlen(pItem->pszText) + 1)]);
	...
	{
		ATLASSERT(!::IsWindow(this->m_hWnd)); // Before control is created, please!
...
#	endif
		...
		UINT nID = static_cast<UINT>(this->GetDlgCtrlID());
		m_ctrlTree.Create(this, 1, this->m_hWnd, &TBase::rcDefault, NULL, dwStyle, 0, nID);
		ATLASSERT(m_ctrlTree.IsWindow());
		m_ctrlTree.ModifyStyle(0, TVS_NOHSCROLL | TVS_FULLROWSELECT); // we need these

		// Create the header control
		m_ctrlHeader.Create(this, 2, this->m_hWnd, &TBase::rcDefault, NULL, m_dwHeaderStyle);
	...
	void _CalcColumnSizes()
	{
		...
		if (m_cxHeader - cxClient >= 0) {
			...
			si.nMax = m_cxHeader;
			si.nMin = 0;
			si.nPage = static_cast<UINT>(cxClient);
			si.nPos = this->GetScrollPos(SB_HORZ); // Preserve scroller position
			this->SetScrollInfo(SB_HORZ, &si, TRUE); // Setup scrollbar params
			this->ModifyStyle(0, WS_HSCROLL, SWP_FRAMECHANGED);
			...
			if (si.nPos) { // Scroll position is non-zero. code is stolen from OnHScroll()
				...
				m_ctrlHeader.SetWindowPos(
				    NULL, -si.nPos, 0, abs(si.nPos) + ::GetSystemMetrics(SM_CXVSCROLL) + 1 + rcTree.right - rcTree.left,
				    rcHeader.bottom - rcHeader.top, SWP_SHOWWINDOW);
			}
		} else {
			this->SetScrollPos(SB_HORZ, 0, TRUE);
			this->ModifyStyle(WS_HSCROLL, 0, SWP_FRAMECHANGED);
			m_nOffset = 0;
			this->Invalidate();
		}
	}
	...
	LPARAM OnHScroll(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		int nCurPos = this->GetScrollPos(SB_HORZ); // Current scrollingposition
		...
		this->GetScrollRange(SB_HORZ, &nScrollMin, &nScrollMax);
		...
		switch (nSBCode) {
			...
			case SB_THUMBTRACK: // Drop the scrollbarbutton
				...
				if (nPos == 0) {
					...
					nCurPos = std::min(StretchWidth(nPos, nWidthLine), static_cast<long>(nScrollMax));
				}
		}

		// Move the scrollbarbutton to the position (graphically)
		this->SetScrollPos(SB_HORZ, nCurPos, TRUE);
		m_nOffset = -nCurPos;
		...
		if (rcTree.right - rcTree.left != 0) {
			...
			m_ctrlHeader.SetWindowPos(
			    NULL, m_nOffset, 0, abs(m_nOffset) + iVSWidth + rcTree.right - rcTree.left,
			    rcHeader.bottom - rcHeader.top, SWP_SHOWWINDOW);
		}
	...
	LRESULT OnTreeFixButtonHit(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& bHandled)
	{
		...
		if (x > rcClient.right - rcClient.left)
			x = rcClient.right - 40;
		TVHITTESTINFO hti;
		hti.pt.x = x;
		hti.pt.y = y;
		...
		if (hti.flags == TVHT_ONITEMRIGHT)
			return m_ctrlTree.SelectItem(hti.hItem);
		bHandled = FALSE;
		return 0;
	}
	...
	DWORD OnItemPrePaint(int /*idCtrl*/, LPNMCUSTOMDRAW lpNMCustomDraw)
	{
		...
		if (m_dwExStyle & TLVS_EX_SELTOHEADER)
			m_rcItem.right = m_cxHeader;
			...
			if (pVal && pVal->GetSize() > 0) {
				...
				if (pItem->mask & TLVIF_COLOR) {
					if (pItem->clrText != CLR_NONE)
						pTVCustomDraw->clrText = pItem->clrText;
					if (pItem->clrBack != CLR_NONE)
						pTVCustomDraw->clrTextBk = pItem->clrBack;
				}
			}
		}
	...
	void UpdateLayout()
	{
		...
		if (this->GetStyle() & WS_HSCROLL)
			m_nOffset = -this->GetScrollPos(SB_HORZ); // read scrollbar position
	...
	void DrawTreeItem(LPNMTVCUSTOMDRAW lptvcd, UINT iState, const RECT& rcItem)
	{
		...
		for (int i = 0; i < cnt; i++) {
			...
			if (pItem->mask & TLVIF_IMAGE) {
				...
				il.DrawEx(
				    pItem->iImage, dc, rc.left, rc.top, std::min(static_cast<LONG>(cx), rc.right - rc.left), cy,
			...
			if (pItem->mask & TLVIF_TEXT) {
				...
				if (pItem->mask & TLVIF_COLOR) {
					if (bSelected) {
						...
						if (pItem->clrBack != CLR_NONE) {
							...
							if (((*pVal)[0]->mask & TLVIF_COLOR) && ((*pVal)[0]->clrBack != CLR_NONE))
								clrBack = (*pVal)[0]->clrBack;
				...
				dc.DrawText(pItem->pszText, -1, &rc, DT_VCENTER | DT_SINGLELINE | DT_WORD_ELLIPSIS | format);
