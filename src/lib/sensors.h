...
extern WM_SENSORS_EXPORT const char* libsensors_version;
...
WM_SENSORS_EXPORT int sensors_init(FILE* input);

/* Clean-up function: You can't access anything after
   this, until the next sensors_init() call! */
WM_SENSORS_EXPORT void sensors_cleanup(void);

/* Parse a chip name to the internal representation. Return 0 on success, <0
   on error. */
WM_SENSORS_EXPORT int sensors_parse_chip_name(const char* orig_name, sensors_chip_name* res);

/* Free memory allocated for the internal representation of a chip name. */
WM_SENSORS_EXPORT void sensors_free_chip_name(sensors_chip_name* chip);
...
WM_SENSORS_EXPORT int sensors_snprintf_chip_name(char* str, size_t size,
...
/* This function returns the adapter name of a bus,
   ...
   it returns NULL */
WM_SENSORS_EXPORT const char* sensors_get_adapter_name(const sensors_bus_id* bus);
...
WM_SENSORS_EXPORT char* sensors_get_label(const sensors_chip_name* name,
			const sensors_feature *feature);

/* Read the value of a subfeature of a certain chip. Note that chip should not
   contain wildcard values! This function will return 0 on success, and <0
   on failure.  */
WM_SENSORS_EXPORT int sensors_get_value(const sensors_chip_name* name, int subfeat_nr,
		      double *value);

/* Set the value of a subfeature of a certain chip. Note that chip should not
   contain wildcard values! This function will return 0 on success, and <0
   on failure. */
WM_SENSORS_EXPORT int sensors_set_value(const sensors_chip_name* name, int subfeat_nr,
		      double value);

/* Execute all set statements for this particular chip. The chip may contain
   wildcards!  This function will return 0 on success, and <0 on failure. */
WM_SENSORS_EXPORT int sensors_do_chip_sets(const sensors_chip_name* name);
...
WM_SENSORS_EXPORT const sensors_chip_name* sensors_get_detected_chips(const sensors_chip_name
						    *match, int *nr);
...
/* This returns all main features of a specific chip. nr is an internally
   ...
   data structures. */
WM_SENSORS_EXPORT const sensors_feature* sensors_get_features(const sensors_chip_name *name, int *nr);
...
/* This returns all subfeatures of a given main feature. nr is an internally
   ...
   Do not try to change the returned structure; you will corrupt internal
   data structures. */
WM_SENSORS_EXPORT const sensors_subfeature*
